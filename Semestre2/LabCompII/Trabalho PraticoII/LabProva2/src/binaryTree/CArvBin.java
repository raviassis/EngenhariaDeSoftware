...
public class CArvBin {
  ...
  public void insereFor(int item) {
	  if (this.vazia()) this.raiz = new CNo(item);
	  else {
		  CNo no = this.raiz;
...
public void imprimir(int Ordem)
 ...
 public int profundidade(int item) {
  ...
  public int pesquisa(int item) {
	  return retornaNo(item, this.raiz).item;
  }
  
  public int pesquisaFor(int item) {
	  if (this.vazia()) return 0;
	  else {
		  CNo no = this.raiz;
		  int val = 0;
		  ...
		  while(!achou) {
			  ...
			  else if ( item < no.item) no = no.esq;
			  else if ( item > no.item) no = no.dir;			  
		  }
