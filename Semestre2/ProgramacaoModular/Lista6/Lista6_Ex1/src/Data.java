...
public class Data implements Ordenavel {
	...
	public Data(int y, int m, int d) throws ExcecaoDataInvalida{
	...
	private void setAno(int ano) throws ExcecaoDataInvalida {
		if (ano < 1) throw new ExcecaoDataInvalida();
		this.ano = ano;
	}

	private void setMes(int mes) throws ExcecaoDataInvalida {
		if ( mes < 1 || mes > 12) throw new ExcecaoDataInvalida();
		this.mes = mes;
	}

	private void setDia(int dia) throws ExcecaoDataInvalida {
		if ( dia < 1 || dia > 31) throw new ExcecaoDataInvalida();
		if (this.getMes() != 2 && 
			(this.getMes() == 4 || this.getMes() == 6 || this.getMes() == 9 || this.getMes() == 11) &&
			dia == 31)
			throw new ExcecaoDataInvalida();
		if(this.getMes() == 2) {
			if ( this.eBissexto() && dia > 29) throw new ExcecaoDataInvalida();
			else if ( !this.eBissexto() &&  dia > 28) throw new ExcecaoDataInvalida();
		}
		this.dia = dia;
	}
